...
class NotificationService: NotificationServiceProtocol {
    ...
    // 알림 목록 조회
    func getNotifications() -> Observable<[NotificationModel]> {
        return apiClient.requestWithDateDecoding(APIEndpoint.getNotifications)
    ...
    // 알림 읽음 처리
    func markNotificationAsRead(id: String) -> Observable<Bool> {
        return apiClient.request(APIEndpoint.markNotificationAsRead(id: id))
    ...
    // 모든 알림 읽음 처리
    func markAllNotificationsAsRead() -> Observable<Bool> {
        return apiClient.request(APIEndpoint.markAllNotificationsAsRead)
    ...
    // 알림 삭제
    func deleteNotification(id: String) -> Observable<Bool> {
        return apiClient.request(APIEndpoint.deleteNotification(id: id))
            .map { (response: APIResponse<EmptyResponse>) -> Bool in
    ...
    // 읽지 않은 알림 수 조회
    func getUnreadCount() -> Observable<Int> {
        return apiClient.request(APIEndpoint.getUnreadCount)
    ...
    // 테스트용 여행 리마인더 알림 생성
    func createJourneyReminder(journeyId: String) -> Observable<NotificationModel> {
        return apiClient.requestWithDateDecoding(APIEndpoint.createJourneyReminder(journeyId: journeyId))
    ...
    // 테스트용 날씨 알림 생성
    func createWeatherAlert(journeyId: String) -> Observable<NotificationModel> {
        return apiClient.requestWithDateDecoding(APIEndpoint.createWeatherAlert(journeyId: journeyId))
            .map { (response: APIResponse<NotificationResponse>) -> NotificationModel in
                guard let notification = response.data?.notification else {
                    throw NetworkError.invalidResponse
                }
