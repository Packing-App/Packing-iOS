...
class EmailSignUpViewController: UIViewController, StoryboardView {
    ...
    func bind(reactor: EmailSignUpReactor) {
        ...
        confirmPasswordTextField.rx.text.orEmpty
            .map { Reactor.Action.updatePasswordConfirm($0) }
            .bind(to: reactor.action)
            .disposed(by: disposeBag)
        ...
        nextButton.rx.tap
            .map { Reactor.Action.goToVerification }
            .bind(to: reactor.action)
            .disposed(by: disposeBag)
        ...
        reactor.state.map { $0.emailErrorMessage }
            .distinctUntilChanged()
            .filter { $0 != nil }
        ...
        reactor.state.map { $0.passwordErrorMessage }
            .distinctUntilChanged()
            .filter { $0 != nil }
            .subscribe(onNext: { [weak self] message in
        ...
        reactor.state.map { $0.isPasswordConfirmValid }
            .distinctUntilChanged()
            .subscribe(onNext: { [weak self] isValid in
        ...
        reactor.state.map { $0.passwordConfirmErrorMessage }
            .distinctUntilChanged()
            .filter { $0 != nil }
            .subscribe(onNext: { [weak self] message in
        ...
        // Name validation
        reactor.state.map { $0.isNameValid }
            .distinctUntilChanged()
            .subscribe(onNext: { [weak self] isValid in
        ...
        reactor.state.map { $0.nameErrorMessage }
            .distinctUntilChanged()
            .filter { $0 != nil }
            .subscribe(onNext: { [weak self] message in
        ...
        // Form Validation - Next Button Enable
        ...
        // Navigation
        reactor.state.map { $0.isNavigatingToVerification }
            .distinctUntilChanged()
            .filter { $0 }
            .subscribe(onNext: { [weak self] _ in
                ...
                let verificationReactor = EmailVerificationReactor(
                ...
                verificationVC.reactor = verificationReactor
...
extension EmailSignUpViewController: UITextFieldDelegate {
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        case 3:
            dismissKeyboard()
            if nextButton.isEnabled {
                // Value of type 'Reactive<UIBarButtonItem>' has no dynamic member 'tap' using key path from root type 'UIBarButtonItem'
